...
export class ChatService {
    ...
    try {
      ...
      const conv = convos.map((conv) => {
        ...
        if (group) {
          ...
          const { userName, photo } = conv.ChatroomUsers.find(
  ...
  async getConversationMessages(userId: number, roomId: number) {
    try {
      ...
      const filtredMessages = await Promise.all(
  ...
  async getChatMembers(userId: number, roomId: number) {
    try {
      ...
      console.log('users:', groupChatUsers);
      if (groupChatUsers.ChatroomUsers.find((user) => user.role === 'OWNER')) {
      }

      const users = groupChatUsers.ChatroomUsers.map((user) => {
        // const owner = user[0].find(user => user.role === 'OWNER')
      ...
      .then(async (user) => {
        ...
        if (user.role !== 'OWNER' && user.role !== 'ADMIN')
          throw new ForbiddenException(
            'You are not allowed to add admins to this room',
          );
        if (
          !user.chatroom.ChatroomUsers.find((user) => user.userId === memberId)
        )
