...
export class UserController {
  ...
  async getFriendship(@Query() param: userIdDTO, @Req() req: any, @Res() res) {
    res.json(
      await this.userService.getFriendship(req.user.id, parseInt(param.id)),
    );
  }
  ...
  async getBlocks(@Req() req, @Query() param: userIdDTO, @Res() res) {
    res.json(
      await this.userService.getBlockStatus(req.user.id, parseInt(param.id)),
    );
  }
  ...
  async unblockUser(@Req() req, @Query() param: userIdDTO, @Res() res) {
    res.json(
      await this.userService.unblockUser(req.user.id, parseInt(param.id)),
    );
  }
