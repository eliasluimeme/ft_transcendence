      ...
      try {
        ...
        if (user) {
          ...
          if (user.id === userId)
            ...
            user.sentRequests &&
            user.sentRequests.find((request) => request.receiverId === userId)
          )
            return {
              id: user.id,
              userName: user.userName,
              ...
              level: user.level.level,
              rank: rank,
              friend: user.sentRequests[0].status,
            };
          else if (
            user.receivedRequests &&
            user.receivedRequests.find((request) => request.senderId === userId)
          )
            return {
              id: user.id,
              userName: user.userName,
              ...
              level: user.level.level,
              rank: rank,
              friend: user.receivedRequests[0].status,
            };
          else
            return {
              id: user.id,
