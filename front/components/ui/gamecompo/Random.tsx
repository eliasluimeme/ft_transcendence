...
const Random = (props:any) => {
  const newGameVsRandom = () => {
    props.socket.emit('newGamePlayer', '');
    props.setStatus(1);
  }
  return (
    <div className="grid grid-cols-2 w-full h-full ">
      <div className="col-start-1 flex flex-col items-center justify-center">
        ...
        <div>Online Game</div>
        <button onClick={newGameVsRandom} className="border w-[40%] h-[20%] rounded-lg bg-[#EDF0F4] text-[#3B2D59] shadow-sm shadow-white hover:text-opacity-70">
          Play
        </button>
      </div>
